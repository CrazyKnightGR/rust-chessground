...
use crate::pieces::{Drag, Figurine};
use crate::util::pos_to_square;
...
impl Pockets {
    ...
    pub fn set_pockets(&mut self, material: Material, color: Color) {
        ...
        if material.pawns > 0 {
            self.figurines.push(new_figurine(
                Piece {
                    color,
                    role: Role::Pawn,
                },
    ...
    pub(crate) fn drag_mouse_up(&mut self, ctx: &EventContext) {
        let (piece, dest) = if let Some(drag) = self.drag.take() {
...
fn new_figurine(piece: Piece, x: f64, y: f64, now: Instant) -> Figurine {
    Figurine {
        square: Square::A1, // Dummy square.
        piece,
        start: (x, y),
